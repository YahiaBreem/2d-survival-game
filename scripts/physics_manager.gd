...
func _ready() -> void:
	...
	_fg = get_tree().get_first_node_in_group("layer_main") as TileMapLayer
...
func _check_cell(cell: Vector2i) -> void:
	...
	if _fg.get_cell_source_id(cell) == -1:
		return
	var atlas: Vector2i = _fg.get_cell_atlas_coords(cell)
...
func _physics_process(delta: float) -> void:
	if _falling.is_empty() or _fg == null:
		return
	...
	for entry in _falling:
		...
		while entry["accum"] >= 1.0:
			...
			if _fg.get_cell_source_id(below) == -1:
				# Air below — fall one cell
				entry["cell"] = below
			else:
				# Landed — place the block and stop
				_fg.set_cell(entry["cell"], 0, entry["atlas"])
